...
def build_demo_pose(device, dtype):
    ...
    # 实验参数：非对称行走/挥手姿态，便于观察 LBS 各阶段差异
    global_orient[0] = torch.tensor([0.0, 0.25, 0.0], dtype=dtype, device=device)  # 躯干微转
    set_joint_pose("left_shoulder", [0.0, 0.0, 1.2])   # 左臂侧平举
    set_joint_pose("right_shoulder", [0.5, 0.0, -0.6]) # 右臂前抬
    set_joint_pose("left_elbow", [0.0, -0.8, 0.0])     # 左肘弯曲
    set_joint_pose("right_elbow", [0.0, 1.0, 0.0])     # 右肘弯曲
    set_joint_pose("left_hip", [-0.4, 0.0, 0.15])      # 左髋后伸
    set_joint_pose("right_hip", [0.55, 0.0, -0.1])     # 右髋前屈
    set_joint_pose("left_knee", [0.15, 0.0, 0.0])      # 左膝微弯
    set_joint_pose("right_knee", [0.7, 0.0, 0.0])      # 右膝明显弯曲
